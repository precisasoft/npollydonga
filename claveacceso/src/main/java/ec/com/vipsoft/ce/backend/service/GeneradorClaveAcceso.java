...
public class GeneradorClaveAcceso implements Serializable {
    ...
    public String generarClaveAccesoFactura(Date fecha, String ruc, Integer sucursal, Integer puntoEmision, boolean enPruebas, long numerocomprobante, int secuenciaca) {
        return generarClaveAcceso(fecha,ruc, sucursal, puntoEmision, enPruebas, numerocomprobante, secuenciaca, "01");
    }

    public String generarClaveAccesoNotaCredito(Date fecha,String ruc, Integer sucursal, Integer puntoEmision, boolean enPruebas, long numerocomprobante, int secuenciaca) {
        return generarClaveAcceso(fecha,ruc, sucursal, puntoEmision, enPruebas, numerocomprobante, secuenciaca, "04");
    }

    public String generarClaveAccesoNotaDebito(Date fecha,String ruc, Integer sucursal, Integer puntoEmision, boolean enPruebas, long numerocomprobante, int secuenciaca) {
        return generarClaveAcceso(fecha,ruc, sucursal, puntoEmision, enPruebas, numerocomprobante, secuenciaca, "05");
    }

    public String generarClaveAccesoGuiaRemision(Date fecha,String ruc, Integer sucursal, Integer puntoEmision, boolean enPruebas, long numerocomprobante, int secuenciaca) {
        return generarClaveAcceso(fecha,ruc, sucursal, puntoEmision, enPruebas, numerocomprobante, secuenciaca, "06");
    }

    public String generarClaveAccesoComprobanteRetencion(Date fecha,String ruc, Integer sucursal, Integer puntoEmision, boolean enPruebas, long numerocomprobante, int secuenciaca) {
        return generarClaveAcceso(fecha,ruc, sucursal, puntoEmision, enPruebas, numerocomprobante, secuenciaca, "07");
    }

    
    private String generarClaveAcceso(Date fecha,@Pattern(regexp="[0-9]{13,13}") String ruc,@Min(1) @Max(999) Integer sucursal,@Min(1) @Max(999) Integer puntoEmision, boolean enPruebas, long numerocomprobante,@Min(1) int secuenciaca ,String tipoDocumento) {
        StringBuilder sb = new StringBuilder(representacionTextoFecha(fecha));
...
//    }
    private String representacionTextoFecha(Date fe) {
        SimpleDateFormat sdf = new SimpleDateFormat("ddMMyyyy");
        if(fe==null){
        	return sdf.format(new Date());
        }else{
        	return sdf.format(fe);	
        }
        
    }

    public String generarClaveAccesoContingencia(Date fecha,String claveAccesoNormal, String siguienteClaveContingenciaRucEmisor) {
        StringBuilder sb = new StringBuilder();
        sb.append(representacionTextoFecha(fecha));
        sb.append(claveAccesoNormal.subSequence(9, 10));
        sb.append(siguienteClaveContingenciaRucEmisor);        
        sb.append("2");
        sb.append(obtenerSumaPorDigitos(sb.toString()));
        return sb.toString();
    }

    public String generarClaveAccesoContingencia(String claveAccesoNormal, String siguienteClaveContingenciaRucEmisor) {
        StringBuilder sb = new StringBuilder();
        sb.append(claveAccesoNormal.subSequence(0, 10));
        sb.append(siguienteClaveContingenciaRucEmisor);        
        sb.append("2");
        sb.append(obtenerSumaPorDigitos(sb.toString()));
        return sb.toString();
